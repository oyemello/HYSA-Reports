...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "frontend" / "public" / "fedrate"

FRED_API = "https://api.stlouisfed.org/fred/series/observations"
SERIES = {
    "effective": "DFF",        # Effective Fed Funds Rate
    "upper": "DFEDTARU",       # Target range upper
    "lower": "DFEDTARL",       # Target range lower
}
...
def main():
    ap = argparse.ArgumentParser(description="Build static Fed rate site from FRED data.")
    ap.add_argument("--api-key", default=os.environ.get("FRED_API_KEY"), help="FRED API key (or set FRED_API_KEY env var)")
    ap.add_argument("--start", default=None, help="Start date YYYY-MM-DD (default: 10 years ago)")
    ap.add_argument("--end", default=None, help="End date YYYY-MM-DD (default: today)")
    ap.add_argument(
        "--out-dir",
        default=str(DEFAULT_OUTPUT_DIR),
        help="Directory to write static assets (default: frontend/public/fedrate)",
    )
    args = ap.parse_args()

    if not args.api_key:
        raise SystemExit("Please provide a FRED API key via --api-key or FRED_API_KEY env var.")

    today = dt.date.today()
    start = args.start or (today.replace(year=today.year - 10).isoformat())
    ...
    for key, sid in SERIES.items():
        data[key] = fred_series(sid, args.api_key, start, end)

    # Latest date among series (if any empty, ignore)
    latest = max([s[-1]["date"] for s in data.values() if s], default=end)
    ...
    json_path = output_dir / "fed_data.json"
    ...
    print(
        "\nPreview the chart by opening the HTML file directly, or serve the directory:\n"
        f"  python -m http.server -d {output_dir} 8000\n"
