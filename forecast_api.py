...
def get_forecast():
    ...
    institution_apy = max([r["apy"] for r in competitor_apy]) if competitor_apy else 0.0
    ...
    # Scenario 1: Federal Rate (cost of funds = fedfunds, NIM = gs10-fedfunds)
    if fedfunds is None or gs10 is None:
        # Fallback: use institution_apy for both scenarios
        fedfunds = institution_apy
        gs10 = institution_apy
    nim_fed = [round(gs10 - fedfunds, 2)] * 3
    profit_fed = [round((n/12)*b*((gs10-fedfunds)/100), 2) for n, b in zip(months, balances)]
    # Scenario 2: American Express (cost of funds = institution_apy, NIM = gs10-institution_apy)
    nim_amex = [round(gs10 - institution_apy, 2)] * 3
    profit_amex = [round((n/12)*b*((gs10-institution_apy)/100), 2) for n, b in zip(months, balances)]
    summary = get_gemini_summary(fred_data, competitor_apy, institution_apy, {
        "months": months,
        "balances": balances,
        "federal_rate": {
            "cost_of_funds": [fedfunds]*3,
            "nim": nim_fed,
            "profit": profit_fed
        },
        "amex": {
            "cost_of_funds": [institution_apy]*3,
            "nim": nim_amex,
            "profit": profit_amex
        }
    })
